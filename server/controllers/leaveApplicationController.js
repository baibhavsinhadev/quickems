...
export const createLeave = async (req, res) => {
    try {
        ...
        if (employee.isDeleted) {
            return res.status(403).json({ success: false, message: "Your account is deactivated. You cannot apply for leave" });
        };
        ...
        if (new Date(startDate) <= today || new Date(endDate) <= today) {
            return res.status(400).json({ success: false, message: "Leave dates must be in the future" });
        };
        ...
        await inngest.send({
            name: "leave/application.created",
            data: {
...
export const getLeaves = async (req, res) => {
    try {
        ...
        return res.status(500).json({ success: false, message: "Failed to fetch leave" });
    }
}
