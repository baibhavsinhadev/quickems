    ...
    const sidebarContent = (
        <>
                ...
                <div className="mx-3 mt-4 mb-1 p-3 rounded-lg bg-white/3 border border-white/4">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                            ...
                            <p className="text-[11px] text-slate-500 truncate">{role === "ADMIN" ? "Administration" : "Employee"}</p>
                        </div>
                    </div>
                </div>
            )}
                ...
                {loading ? (
                    ...
                    return (
                        <Link key={item.name} to={item.href} className={`group flex items-center gap-3 px-3 py-2.5 rounded-md text-[13px] font-medium transition-all duration-150 relative ${isActive ? "bg-indigo-500/12 text-indigo-300" : "text-slate-300 hover:text-white hover:bg-white/4"}`}>
                            {isActive && (
                                <div className="absolute left-0 top-1/2 -translate-y-1/2 w-0.76 h-5 rounded-r-full bg-indigo-500" />
                            )}

                            <Icon className={`w-4.25 h-4.25 shrink-0 ${isActive ? "text-indigo-300" : "text-gray-400 group-hover:text-slate-300"}`} />
            ...
            <div className="p-3 border-t border-white/6">
                <button onClick={handleLogout} className="flex items-center gap-3 w-full px-3 py-2.5 rounded-md text-[13px] font-medium text-slate-400 hover:text-rose-400  cursor-pointer hover:bg-rose-500/8 transition-all duration-150">
